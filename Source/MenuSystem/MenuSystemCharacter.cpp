...
void AMenuSystemCharacter::CreateGameSession()
{
	...
	OnlineSessionInterface->AddOnEndSessionCompleteDelegate_Handle(CreateSessionCompleteDelegate);

	// Create new session after destroying 
	...
	SessionSettings->bUsesPresence = true; //Steams netcode requires closer region
...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	...
	else
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Green, FString(TEXT("Failed To Create Session")));
		}
	}
}
...
void AMenuSystemCharacter::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
	...
	if (PlayerController)
	{
		//Travel to the level and join the game
